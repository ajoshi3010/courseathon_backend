...
router.get('/:tutorId/courses/:courseId/students', async (req, res) => {
  try {
    ...
    // Populate enrolled students with only their names
    await course.populate({
      path: 'enrolledStudents',
      select: 'name -_id' // Select only the name field and exclude _id
    }).execPopulate();

    // Extract the names from populated enrolledStudents
    const studentNames = course.enrolledStudents.map(student => student.name);
