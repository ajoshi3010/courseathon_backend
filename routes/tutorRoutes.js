...
router.put('/:tutorId/aboutme', async (req, res) => {
  ...
  await tutor.save();
  res.status(200).json({ message: 'aboutMe updated successfully', tutor });
  if (!tutor) {
...
router.put('/:tutorId/imageLink', async (req, res) => {
  try {
    ...
    await tutor.save();
    res.status(200).json({ message: 'imageLink updated successfully', tutor });
    if (!tutor) {
      return res.status(404).json({ message: 'Tutor not found' });
    }
  } catch (error) {
...
router.put('/:tutorId/name', async (req, res) => {
  const tutorId = req.params.tutorId;
    ...
    tutor.name=name;
    await tutor.save();
    res.status(200).json({ message: 'name updated successfully', tutor });
    if (!tutor) {
      return res.status(404).json({ message: 'Tutor not found' });
    }
});
...
router.get('/:tutorId/profile', async (req, res) => {
  try {
    const tutorId = req.params.tutorId;
    console.log(tutorId);
    const tutor = await Tutor.findOne({ userId: tutorId });
    ...
    const { name, aboutMe, imageLink } = tutor;
    res.json({ name, aboutMe, imageLink });
  } catch (err) {
    console.error(err);
